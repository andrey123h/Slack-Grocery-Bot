...
/**
 * Holds the raw JSON payloads from Block Kit Builder:
...
public class ViewPayloads {
    ...
    // Everything up to (but not including) the dynamic item blocks:
    // NOTE: We add a trailing comma after the “*Current Defaults:*” section so that
    // when no items exist, the next footer object can follow directly without causing
    // a JSON‐syntax error (missing comma).
    ...
    // Everything after the dynamic item blocks:
    // Since HOME_JSON_HEADER already ends in a comma, we can directly begin with another block.
    ...
    // The modal JSON is used exactly as‐is when opening “Add / Edit Default”:
