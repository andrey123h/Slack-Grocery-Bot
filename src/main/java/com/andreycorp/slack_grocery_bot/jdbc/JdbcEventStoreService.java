...
/**
 * JJDBC‐backed implementation of EventStore that persists and loads Slack message and reaction events.
 */
...
public class JdbcEventStoreService implements EventStore {
    ...
    public List<MessageEvent> fetchMessagesSince(String fromTs) {
        String sql = "SELECT team_id,user_id,channel_id,text,ts FROM message_event "
                + "WHERE team_id = ? AND ts_epoch >= ? ORDER BY ts_epoch";
    ...
    public List<ReactionEvent> fetchReactionsSince(String fromTs) {
        String sql = "SELECT team_id,user_id,reaction,channel_id,ts FROM reaction_event "
                + "WHERE team_id = ? AND ts_epoch >= ? ORDER BY ts_epoch";
